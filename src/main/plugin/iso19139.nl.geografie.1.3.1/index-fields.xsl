...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
	...
	<xsl:template match="*" mode="metadata" priority="10">

        <!-- this template has a higher priority then it's duplicate in iso19139, 
        all dutch profile changes are managed here, note that you will also
        have to add the dutch-profile-index-fields in config-lucene.xml and config-summary.xml -->
        ...
        <xsl:for-each select="gmd:identificationInfo//gmd:MD_DataIdentification|
			...
			<xsl:for-each select="gmd:distributorContact/gmd:CI_ResponsibleParty/gmd:organisationName/gco:CharacterString|gmd:distributorContact/gmd:CI_ResponsibleParty/gmd:organisationName/gmx:Anchor">
                ...
                <xsl:variable name="address" select="string-join(../../gmd:contactInfo/*/gmd:address/*/(
                    ...
                    <Field name="responsibleParty"
                           string="{concat($roleTranslation, '|distributor|', ., '|', $logo, '|',  string-join($email, ','), '|', $individualName, '|', $positionName, '|', $address, '|', string-join($phone, ','))}"
                           store="true" index="false"/>
            </xsl:for-each>
...
-->
      ...
      <xsl:for-each select="srv:serviceType/gco:LocalName">
            ...
            <xsl:for-each select="srv:operatesOn/@xlink:href">
                ...
                <Field  name="operatesOn" string="{tokenize(tokenize(string(.),'&amp;id=')[2],'&amp;')[1]}" store="true" index="true"/>
            </xsl:for-each>
        ...
        <xsl:for-each select="gmd:distributionInfo/gmd:MD_Distribution">
            ...
            <xsl:for-each select="gmd:transferOptions/gmd:MD_DigitalTransferOptions">
                ...
                <xsl:for-each select="gmd:onLine/gmd:CI_OnlineResource[gmd:linkage/gmd:URL!='']">
                    ...
                    <xsl:variable name="wfsLinkNoProtocol" select="contains(lower-case($linkage), 'service=wfs') and not(string($protocol))" />
                    <xsl:variable name="wcsLinkNoProtocol" select="contains(lower-case($linkage), 'service=wcs') and not(string($protocol))" />



                    <!-- ignore empty downloads -->
                    <xsl:if test="string($linkage)!='' and not(contains($linkage,$download_check))">
						<xsl:if test="string($protocol)!='dataset'">
                    ...
                    <xsl:if test="contains($protocol, 'OGC:WFS') or contains($protocol, 'OGC:WCS') or contains($protocol, 'INSPIRE Atom') or $wfsLinkNoProtocol or $wcsLinkNoProtocol">
                        <Field name="download" string="true" store="false" index="true"/>
                    </xsl:if>

                    <xsl:if test="contains($protocol, 'OGC:WMS') or contains($protocol, 'OGC:WMTS') or$wmsLinkNoProtocol">
                        <Field name="dynamic" string="true" store="false" index="true"/>
                    </xsl:if>

                    <xsl:if test="not(contains($protocol, 'WWW:DOWNLOAD') or contains($protocol, 'OGC:WFS') or contains($protocol, 'OGC:WCS') or contains($protocol, 'INSPIRE Atom') or $wfsLinkNoProtocol or $wcsLinkNoProtocol or contains($protocol, 'OGC:WMS') or $wmsLinkNoProtocol)">
			...
			<!-- index distributor -->
			<xsl:for-each select="gmd:distributor/gmd:MD_Distributor/gmd:distributorContact/gmd:CI_ResponsibleParty/gmd:organisationName/gco:CharacterString|gmd:distributorContact/gmd:CI_ResponsibleParty/gmd:organisationName/gmx:Anchor">
        ...
        <xsl:for-each select="gmd:contact/*/gmd:organisationName/gco:CharacterString|gmd:contact/*/gmd:organisationName/gmx:Anchor">
            ...
            <Field name="responsibleParty"
                               string="{concat($roleTranslation, '|metadata|', ., '|', $logo, '|', string-join($email, ','), '|', $individualName, '|', $positionName, '|', $address, '|', string-join($phone, ','),'|',string-join($web,','))}"
                               store="true" index="false"/>
        </xsl:for-each>
